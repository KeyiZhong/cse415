...
def handle_transition(action, new_state, r):
    ...
    q_max = -math.inf
    for a in ACTIONS:
        q_temp = Q_VALUES[(new_state, a)]
        q_max = max(q_max, q_temp)
        # if q_temp > q_max:
        #    q_max = q_temp

    # sample = r(PREVIOUS_STATE, action, new_state) + GAMMA * q_max
    sample = r + GAMMA * q_max
    q_current = Q_VALUES[(PREVIOUS_STATE, action)]
    new_q_value = (1 - ALPHA) * q_current + ALPHA * sample
...
def cal_q(action, new_state, r, prev):
    q_max = -math.inf
    for a in ACTIONS:
        q_temp = Q_VALUES[(new_state, a)]
        q_max = max(q_max, q_temp)
    ...
    # sample = r(PREVIOUS_STATE, action, new_state) + GAMMA * q_max
    sample = r + GAMMA * q_max
    q_current = Q_VALUES[(prev, action)]
    return (1 - ALPHA) * q_current + ALPHA * sample
...
def choose_next_action(s, r, terminated=False):
    ...
    if not (s == INITIAL_STATE):
        ...
        new_qval = cal_q(LAST_ACTION, s, r, PREVIOUS_STATE)  # A bogus value for now.
        # *** ADD OR CHANGE CODE HERE ***
    ...
    for (s_c, a_c) in Q_VALUES.keys():
        if s == s_c:
            q_temp = Q_VALUES[(s_c, a_c)]
            if q_temp > q_max:
                q_max = q_temp
                some_action = a_c
